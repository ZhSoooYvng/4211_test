...
//USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX usb_slist_t g_bus_head;
...
USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX struct usbh_bus g_usbhost_bus[CONFIG_USBHOST_MAX_BUS];
...
//    USB_LOG_DBG("The device selects config %d\r\n", config_index);
    ...
    for (uint8_t i = 0; i < hport->config.config_desc.bNumInterfaces; i++) {
        ...
        {
            USB_LOG_INFO("do  support Class:0x%02x,Subclass:0x%02x,Protocl:0x%02x\r\n",
                    intf_desc->bInterfaceClass,
                    intf_desc->bInterfaceSubClass,
                    intf_desc->bInterfaceProtocol);     
        }
