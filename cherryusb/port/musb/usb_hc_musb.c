...
/* DMA define */
#define MUSB_DMA_OFFSET             0x204
#define USB_DMA_INTR_OFFSET          0x200
#define USB_DMA_CNTL_BASE(channel)    (USB_BASE + MUSB_DMA_OFFSET + 0x10 * (channel-1))
#define USB_DMA_ADDR_BASE(channel)    (USB_BASE + MUSB_DMA_OFFSET+4 + 0x1C * (channel-1))
#define USB_DMA_LEN_BASE(channel)     (USB_BASE + MUSB_DMA_OFFSET+8 + 0x20 * (channel-1))
...
//DMA read packet
static inline void musb_dma_read_packet(struct usbh_bus *bus, uint8_t ep_idx, uint8_t *buffer, uint16_t len)
{
    volatile uint8_t value = 0;
    value =  HWREG(USB_BASE + USB_DMA_INTR_OFFSET);
    HWREG(USB_DMA_ADDR_BASE(1)) = (uint32_t)buffer;
    HWREG(USB_DMA_LEN_BASE(1)) = len;
    HWREG(USB_DMA_CNTL_BASE(1)) = (1<<0)|(0<<1)|(0<<2)|(1<<3)|(ep_idx<<4)|(3<<9);
//    while((HWREGB(USB_BASE + USB_DMA_INTR_OFFSET) & 0x01) == 0x00);
    while((HWREG(USB_BASE + USB_DMA_INTR_OFFSET)&0x01) != 0x01);
    value =  HWREG(USB_BASE + USB_DMA_INTR_OFFSET);
    (void)value;
}
static uint32_t musb_get_fifo_size(uint16_t mps, uint16_t *used)
{
    ...
    for (uint8_t i = USB_TXFIFOSZ_SIZE_8; i <= USB_TXFIFOSZ_SIZE_4096; i++) {
        size = (8 << i);
        if (mps <= size) {
            *used = size;
            return i;
...
#else 
    // 发送新的 设备描述符之前，需要将主机地址设置为新的地址
 //   *((volatile uint8_t *)0x4004c000) = hport->dev_addr ;
   // 不同版本 的 MUSB ip 手册略有不同
    HWREGB(USB_BASE + MUSB_FADDR_OFFSET) = urb->hport->dev_addr;
...
//				printf("urb->ep->bEndpointAddress=%d, %d, %d\r\n", urb->ep->bEndpointAddress, HWREGB(USB_BASE + MUSB_TXFIFOSZ_OFFSET), USB_GET_MAXPACKETSIZE(urb->ep->wMaxPacketSize));
...
int musb_iso_urb_init(struct usbh_bus *bus, uint8_t chidx, struct usbh_urb *urb, uint8_t *buffer, uint32_t buflen)
{
    ...
    if (urb->ep->bEndpointAddress & 0x80) {
        if ((8 << HWREGB(USB_BASE + MUSB_RXFIFOSZ_OFFSET)) < USB_GET_MAXPACKETSIZE(urb->ep->wMaxPacketSize)) {
            USB_LOG_ERR("Ep %02x fifo is overflow\r\n", urb->ep->bEndpointAddress);
            USB_LOG_ERR("%x \r\n",8 << HWREGB(USB_BASE + MUSB_RXFIFOSZ_OFFSET));
        ...
        //HWREGB(USB_BASE + MUSB_IND_RXINTERVAL_OFFSET) = 1;

        HWREGB(USB_BASE + MUSB_IND_RXCSRH_OFFSET) &= (~USB_RXCSRH1_AUTOCL);  // ?????????,???????
        //HWREGB(USB_BASE + MUSB_IND_RXCSRH_OFFSET) |= USB_RXCSRH1_AUTORQ;
        HWREGH(USB_BASE + MUSB_IND_RXMAP_OFFSET) = 0;
        HWREGH(USB_BASE + MUSB_IND_RXMAP_OFFSET) |= USB_GET_MAXPACKETSIZE(urb->ep->wMaxPacketSize);;// must match ep point max size 
        switch (USB_GET_MULT(urb->ep->wMaxPacketSize))
        {
            case 1:{
                HWREGH(USB_BASE + MUSB_IND_RXMAP_OFFSET) |= (0x02 << 11);
                break;
            }
            case 2:{
                HWREGH(USB_BASE + MUSB_IND_RXMAP_OFFSET) |= (0x03 << 11);
                break;
            }
            default:
            break;
        ...
        // 设置接收FIFO 偏移地址
        HWREGH(USB_BASE + MUSB_RXFIFOADD_OFFSET) = 0x08;//0x08;//0x100 ;

        // 设置接收FIFO 大小寄存器
        HWREGB(USB_BASE + MUSB_RXFIFOSZ_OFFSET) = 0;
        HWREGB(USB_BASE + MUSB_RXFIFOSZ_OFFSET) |= 0x09;//0x08 ; //4096
        // 设置接收为 double buffer 模式
        HWREGB(USB_BASE + MUSB_RXFIFOSZ_OFFSET) |= (1<<4);
        
        HWREGH(USB_BASE + MUSB_IND_RXCSRL_OFFSET) |= USB_RXCSRL1_CLRDT | USB_RXCSRL1_FLUSH ;
        /* 关闭 DMA REQ enable */
        HWREGB(USB_BASE + MUSB_IND_RXCSRH_OFFSET) &= (~ USB_RXCSRH1_DMAEN);

        HWREGB(USB_BASE + MUSB_IND_TXCSRH_OFFSET) &= ~USB_TXCSRH1_MODE;     // 设置端点方向为接收
        HWREGB(USB_BASE + MUSB_IND_RXCSRL_OFFSET) = USB_RXCSRL1_REQPKT;     // 请求一个 IN 事务
        HWREGH(USB_BASE + MUSB_RXIE_OFFSET) |= (1 << chidx);                // 设置端点接收中断使能

        USB_LOG_INFO("Set ISO transfer done \r\n");
        //csr = musb_readw(hw_ep->regs, MUSB_RXCSR);
    } else {

        USB_LOG_ERR("No support \r\n");
    }
    musb_set_active_ep(bus, old_ep_index);
...
//__attribute__((section("NO_INIT"))) uint8_t usbbuffer[2000] = {0};
    ...
    for (ep_idx = 1; ep_idx < CONFIG_USBHOST_PIPE_NUM; ep_idx++) {
        if (rxis & (1 << ep_idx)) {
            ...
            if (ep_csrl_status & USB_RXCSRL1_ERROR) {
                ...
                urb->errorcode = -USB_ERR_NAK;
                musb_urb_waitup(urb);
            } else if (ep_csrl_status & USB_RXCSRL1_REQPKT) {
                HWREGB(USB_BASE + MUSB_IND_RXCSRL_OFFSET) &= ~USB_RXCSRL1_REQPKT;
                urb->errorcode = -USB_ERR_STALL;
                musb_urb_waitup(urb);
            }else if(ep_csrl_status & USB_RXCSRL1_FULL)
            {
                uint32_t size = HWREGH(USB_BASE + MUSB_IND_RXCOUNT_OFFSET);   
                USB_LOG_ERR("FIFO FULL ,Curren fifo cnt is %d \r\n",size);
                while(1);
            } 
                ...
                if ((size < USB_GET_MAXPACKETSIZE(urb->ep->wMaxPacketSize)) || (urb->transfer_buffer_length == 0)) {
                    ...
                    case USB_ENDPOINT_TYPE_ISOCHRONOUS:{
                        ...
                        if(size > USB_GET_MAXPACKETSIZE(urb->ep->wMaxPacketSize) * (USB_GET_MULT(urb->ep->wMaxPacketSize) +1))
                        {
                            USB_LOG_ERR("size is larger %d !!!\r\n", size);
                            while(1);
                        }
						
                        // 读取数FIFO中的数据包 CPU or DMA 
						musb_read_packet(bus, ep_idx, urb->transfer_buffer, size);						
//                        musb_dma_read_packet(bus, ep_idx, urb->transfer_buffer, size);						
						
						//cache sync
//						SCB_InvalidateDCache();
//						extern void cc_SCB_InvalidateDCache_by_Addr (uint32_t *addr, int32_t dsize);
//						cc_SCB_InvalidateDCache_by_Addr((uint32_t *)urb->transfer_buffer, size);

#ifdef USBH_UVC_DEMO
				
                        video_iso_process(bus, ep_idx, urb, size); 
#endif
                        HWREGB(USB_BASE + MUSB_IND_RXCSRL_OFFSET) &= ~USB_RXCSRL1_RXRDY;
                        HWREGB(USB_BASE + MUSB_IND_RXCSRL_OFFSET) = USB_RXCSRL1_REQPKT;
                    }break;
                    default:
                        USB_LOG_ERR("type is %x\r\n",USB_GET_ENDPOINT_TYPE(urb->ep->bmAttributes));
                        break;    
                }
            }
        }
...
{
	return (HWREGH(USB_BASE + MUSB_FRAME_OFFSET)&0x7ff);
}
